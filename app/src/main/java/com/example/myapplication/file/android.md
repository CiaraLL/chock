...
没有在 onNewIntent()里面设置 setIntent()，将最新的 intent 设置给这个 activity 实例。                                                  
那么在 onNewIntent()里面的 getIntent()得到的 intent 都是旧数据。


注意：在 onNewIntent()中一定要设置 setIntent(intent)，将系统的 intent 重新给予 A,否则 getIntent()时获取到的是旧的 intent，而不是新的 intent。
...
如果执行了Service，我们是有责任在其请求任务完成后关 闭服务，通过调用 stopSelf0 或 stopService 来结束服务.                                    
IntentService会在执行完所有的请求任务后自行关闭服务，所以我们不必额外调用 stopSelf0 去关闭它.                     
