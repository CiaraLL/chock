...
举例：
1.比如加法，int double float,不能写三个方法；使用泛型；不论传入什么类型，都计算加法
2.编译的时候会提示类型

只能方法中使用通配符 ？extend fruit;继承关系不能超过fruit，fruit的所以子类都可以
...
2. Thread.yield（）
   让当前正在执行d线程停下来一下，把执行机会让给别的在等待的线程，自己回到等待的就绪队列里面，自己也有可能会运行到，
